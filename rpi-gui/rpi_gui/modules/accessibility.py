...
class AccessibleElement:
    """Simplified representation of an accessible node."""
...
def list_running_apps() -> list[dict[str, Any]]:
    """List running applications via AT-SPI, fallback to wmctrl."""
    if not _has_display():
    ...
    if proc.returncode != 0:
        ...
        raise AccessibilityError(
            "AT-SPI unavailable and wmctrl fallback failed"
...
def find_elements(
    ...
    """Search the accessibility tree for matching nodes."""
    if max_results < 1:
        ...
        for node in _iter_descendants(app_node):
            ...
            if text_norm is not None:
                if exact:
                    ...
                    if text_norm not in node_name.lower():
                        continue
...
def list_screens() -> list[dict[str, Any]]:
    """List available screens using xrandr output, with Pillow fallback for primary size."""
    if not _has_display():
        raise AccessibilityError("No GUI display detected (DISPLAY/WAYLAND_DISPLAY missing)")
    ...
    try:
        ...
        img = ImageGrab.grab()  # primary screen fallback
        return [
            {
                "index": 0,
                "name": "primary",
        ...
        raise AccessibilityError(
            "Unable to enumerate screens (xrandr/Pillow failed)"
        ) from exc
...
def list_windows() -> list[dict[str, Any]]:
    ...
    if proc.returncode != 0:
        stderr = (proc.stderr or "").strip()
        raise AccessibilityError(
            "Unable to list windows via wmctrl"
            + (f": {stderr}" if stderr else "")
        )
...
def _resolve_window_id(target: str) -> str:
    ...
    for w in list_windows():
        if w["id"].lower() == target_lower:
            return w["id"]
        if target_lower in w["title"].lower():
            return w["id"]
        if target_lower in w["wm_class"].lower():
            return w["id"]
...
def move_window(target: str, x: int, y: int, width: int | None = None, height: int | None = None) -> dict[str, Any]:
    ...
    proc = _run(["wmctrl", "-ir", win_id, "-e", f"{gravity},{x},{y},{w},{h}"])
    if proc.returncode != 0:
...
def resize_window(target: str, width: int, height: int, x: int | None = None, y: int | None = None) -> dict[str, Any]:
    ...
    proc = _run(["wmctrl", "-ir", win_id, "-e", f"{gravity},{rx},{ry},{width},{height}"])
    if proc.returncode != 0:
