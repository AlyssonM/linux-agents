...
from rpi_gui.modules.input import run_drag
...
@click.command("drag", help="Drag from one point to another")
...
def drag_cmd(from_point: tuple[int, int], to_point: tuple[int, int], duration: float, button: str) -> None:
    try:
        run_drag(
            from_x=from_point[0],
            from_y=from_point[1],
