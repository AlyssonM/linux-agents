...
- **Remove any previous coding instances** that were not closed in the previous session. Use `rpi-term proc list --name codex --json` to find stale agents and `rpi-term proc kill <pid> --tree --json` to kill them and their children.
- You can use `rpi-term proc list --cwd <path to dir>` to find all processes that started in a given directory (your root or operating directory). This can help you clean up the right processes. Just be careful not to kill the `listen` origin server or processes that are required to be long running for your task to be completed successfully.
