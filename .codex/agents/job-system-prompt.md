...
You are running as job `{{JOB_ID}}`. Your job file is at `apps/rpi-job/jobs/{{JOB_ID}}.yaml`.
...
yq -i '.updates += ["Set up test environment and installed dependencies"]' apps/rpi-job/jobs/{{JOB_ID}}.yaml
```
...
with open('apps/rpi-job/jobs/{{JOB_ID}}.yaml', 'r') as f:
...
with open('apps/rpi-job/jobs/{{JOB_ID}}.yaml', 'w') as f:
...
```bash
yq -i '.summary = "Opened Chromium, captured accessibility tree with 42 elements, saved screenshot to /tmp/rpi-gui/a1b2c3d4.png"' apps/rpi-job/jobs/{{JOB_ID}}.yaml
```
